...
export default function VendorOrdersPage() {
  ...
  const stats = useMemo(() => {
    return {
      ...
      enProceso: orders.filter((o) => o.status === "confirmed" || o.status === "shipped").length,
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          <div className="text-xs font-semibold uppercase tracking-wider text-gray-500">En proceso</div>
          <div className="mt-2 text-3xl font-bold text-blue-600">{stats.enProceso}</div>
        </div>
        <div className="rounded-2xl bg-white p-5 shadow-sm">
          <div className="text-xs font-semibold uppercase tracking-wider text-gray-500">Completados</div>
        ...
        <div className="rounded-2xl bg-linear-to-br from-rose-500 to-orange-500 p-5 text-white shadow-sm">
          <div className="text-xs font-semibold uppercase tracking-wider opacity-80">Ingresos</div>
                  ...
                  <tr key={order.id} className="transition hover:bg-gray-50">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        {hasCatalog && (
                          <span title="Incluye productos del catálogo" className="text-xs">🏭</span>
                        )}
                        {isPickup && (
                          <span
              ...
              {filteredOrders.length === 0 && (
                <tr>
                  <td colSpan={7} className="px-6 py-12 text-center text-sm text-gray-400">
                    ...
                    <p className="mt-3">
                      {orders.length === 0 ? "Aún no tienes pedidos." : "No hay pedidos con estos filtros."}
                    </p>
                  </td>
                </tr>
              )}
              ...
              <div className="p-4">
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-gray-500">{formatShortDate(order.created_at)}</div>
                  </div>
                ...
                <div className="mt-3 rounded-xl bg-gray-50 p-3">
                  ...
                  <div className="mt-0.5 truncate font-semibold text-gray-900">{order.customer_name}</div>
            ...
            <div className="space-y-4 p-6">
                  ...
                  <>
                    ...
                    {selectedOrder.delivery_date && selectedOrder.delivery_time_slot && (
                      <div className="mt-2 rounded-lg bg-white border border-gray-200 px-3 py-2 text-xs">
                        📅 Entrega: {selectedOrder.delivery_date} · {selectedOrder.delivery_time_slot}
                      </div>
                    )}
                  </>
                ) : (
                    ...
                    >
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-2">
                          ...
                          <span className="text-xs text-gray-500">{formatCurrency(item.unit_price)} c/u</span>
                        </div>
                      </div>
                      <div className="shrink-0 font-bold text-gray-900">{formatCurrency(item.subtotal)}</div>
                    </div>
                  ))}
                </div>
              ...
              {selectedOrder.has_catalog_items && (
                <div className="rounded-2xl border-l-4 border-purple-500 bg-purple-50 p-4">
                  ...
                  <p className="mt-2 text-sm text-purple-900">
                    Este pedido incluye productos importados del catálogo maestro.
                  </p>
                </div>
              )}
              ...
              <div className="rounded-2xl bg-gray-50 p-4">
                ...
                {selectedOrder.delivery_debt > 0 && (
                  <div className="mt-2 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-xs text-amber-900">
                    ⚠️ Debes S/ {selectedOrder.delivery_debt.toFixed(2)} a Dropship por delivery (se descontará de próxima liquidación)
                  </div>
                )}
              </div>
              ...
              {selectedOrder.tracking_number && (
                <div className="rounded-2xl bg-gray-900 p-4 text-center text-white">
                  ...
                  <div className="mt-1 font-mono text-lg font-bold">{selectedOrder.tracking_number}</div>
                </div>
              )}

              {selectedOrder.status === "confirmed" && selectedOrder.delivery_mode !== "store_pickup" && (
                <div>
                  <label className="block text-sm font-semibold text-gray-700">Tracking de envío</label>
                  <input
                    value={trackingInput}
